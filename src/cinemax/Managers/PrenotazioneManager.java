...
public class PrenotazioneManager {
		...
		try {
			...
			for (int i = 0; i < prenotazioni.size(); i++) {
				...
				String riga = p.getCodice() + ";" +
						...
						sdf.format(p.getProiezione().getData_ora()) + ";" +
	...
	private Proiezione trovaProiezione(String titoloFilm, Date dataOra) {
		for (int i = 0; i < proiezioneManager.proiezioni.size(); i++) {
			...
			if (p.getFilm().getTitolo().equals(titoloFilm) && p.getData_ora().equals(dataOra)) {
				return p;
			}
		}
		return null;
	...
	public boolean creaPrenotazione(Proiezione proiezione, int numeroBiglietti) {
		...
		if (proiezione.getData_ora().before(new Date())) {
		...
		proiezione.setPosti_prenotati(proiezione.getPosti_prenotati() + numeroBiglietti);
	...
	public boolean modificaPrenotazione(String codice, Proiezione nuovaProiezione) {
		...
		if (prenotazione.getProiezione().getData_ora().before(new Date())) {
		...
		if (nuovaProiezione.getData_ora().before(new Date())) {
		...
		prenotazione.getProiezione().setPosti_prenotati(
				prenotazione.getProiezione().getPosti_prenotati() - prenotazione.getNumeroBiglietti());

		// Occupiamo i posti nella nuova proiezione
		nuovaProiezione.setPosti_prenotati(
				nuovaProiezione.getPosti_prenotati() + prenotazione.getNumeroBiglietti());
