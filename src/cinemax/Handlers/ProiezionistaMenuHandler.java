	...
	private Proiezione selezionaProiezioneDaLista(String prompt) {
		...
		while (true) {
			...
			if (filtro.isEmpty()) {
				...
				proiezioni = tutte.stream().filter(p -> p.getFilm() != null && p.getFilm().getTitolo() != null
						&& p.getFilm().getTitolo().toLowerCase().contains(filtroLower)).toList();
			}
		...
		for (int i = 0; i < proiezioni.size(); i++) {
			...
			System.out.printf("%d. %s | %s | %d min | %.2f euro | Prenotazioni: %d%n", i + 1, titolo, dataOra, durata,
					costo, p.getPostiPrenotati());
		}
